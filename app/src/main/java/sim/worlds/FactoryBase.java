...
public abstract class FactoryBase {
    ...
    {
        onePaitent.m_fInitHP = (float) (180+10* Tools.Random().nextGaussian());
        ...
        onePaitent.m_fResistancePowerBase = (float) (0.1+0.01*Tools.Random().nextGaussian());
        onePaitent.m_fResistancePowerGrowthSpeed = (float) (0.04+0.02*Tools.Random().nextGaussian());

        //这两个病毒的参数实际应该由传染给他的人的病毒值来计算变异的
        onePaitent.m_fVirusCopySpeed = (float) (0.8+0.2*Tools.Random().nextGaussian());
        if (onePaitent.m_fVirusCopySpeed <= 0.2)
        {
            onePaitent.m_fVirusCopySpeed = 0.2f;
        }
        onePaitent.m_fVirulence = (float) (1+0.1*Tools.Random().nextGaussian());
